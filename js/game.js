"use strict";
/* =====================================================================
   VERDANT — main loop: camera, movement, AI, waves, pickups, fx
   ===================================================================== */
function gameOver(){
...
function resetGame(){
  ...
  Game.enemies=[]; Game.items=[]; Game.eBullets=[]; Game.corpses=[]; Game.grenades=[]; Game.fx=[];
...
function updateGame(dt,now){
  ...
  /* camera orbit (third-person) + screen shake */
  ...
  let shx=0,shz=0,shy=0;
  if(Game.shake.t>0){
    Game.shake.t-=dt;
    const m=Game.shake.mag*Math.max(0,Game.shake.t/0.35);
    shx=rand(-m,m); shy=rand(-m,m); shz=rand(-m,m);
    if(Game.shake.t<=0) Game.shake.mag=0;
  }
  Game.camera.position.set(
    tx - Math.sin(Game.yaw)*cosP*radius + shx,
    ty + sinP*radius + 1.2 + shy,
    tz - Math.cos(Game.yaw)*cosP*radius + shz
  );
  ...
  /* player rig + aim-down-sights FOV */
  ...
  /* item proximity → prompt + E pickup */
  ...
  if(nearest){
    setPrompt('Pick up '+itemLabel(nearest));
    if(k['e']) collectItem(nearest);
  } else setPrompt(null);
